...
class Estudiante
{
	...
	function traerinscripcionesporrepresentante($idrepresentante)
	{
		$sql = "SELECT p.id as idP, e.id as idE, p.cedula as cedulaE, p.p_nombre as nombreE, p.p_apellido as apellidoE, p.f_nac as fechaE, e.idmadre, e.idpadre, e.estatus as estatus_estudiante, i.* FROM estudiante as e INNER JOIN inscripcion i ON e.id = i.idestudiante INNER JOIN persona as p ON p.id = e.idpersona WHERE i.idrepresentante = '$idrepresentante' GROUP BY e.id";
