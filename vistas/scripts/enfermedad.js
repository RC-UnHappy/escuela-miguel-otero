...
function init() {
  ...
  //Muestra la lista de enfermedad
  listar();

  //Se ejecuta cuando se envia el formulario
  $([formularioEnfermedad]).on("submit", function (event) {
    if ($([formularioEnfermedad])[0].checkValidity()) {
...
function guardaryeditar(event) {
  ...
  var formData = new FormData($([formularioEnfermedad])[0]); //Se obtienen los datos del formulario

  $.ajax({
    url: "../controladores/enfermedad.php?op=guardaryeditar", //Dirección a donde se envían los datos
    ...
    success: function (datos) {
      ...
      if (datos == "true") {
        ...
        Toast.fire({
          type: "success",
          title: "Enfermedad modificada exitosamente :)",
        });
      } else {
        const Toast = Swal.mixin({
          toast: true,
...
function listar() {
  tabla = $("#tblistado").DataTable({
    ...
    ajax: {
      url: "../controladores/enfermedad.php?op=listar",
      type: "GET",
      dataType: "json",
    },
    order: [[0, "desc"]],
...
function mostrar(idenfermedad) {
  $.post(
    "../controladores/enfermedad.php?op=mostrar",
    ...
    function (data) {
      ...
      $("#idenfermedad").val(data.id);
    }
  );
}

//Función para desactivar la enfermedad
function desactivar(idenfermedad) {
  ...
  swalWithBootstrapButtons
    .fire({
      title: "¿Estas seguro?",
      text: "¿Quieres desactivar esta enfermedad?",
      type: "warning",
    ...
    .then((result) => {
      if (result.value) {
        $.post(
          "../controladores/enfermedad.php?op=desactivar",
          { idenfermedad: idenfermedad },
          function (e) {
            if (e == "true") {
              const Toast = Swal.mixin({
              ...
              Toast.fire({
                type: "success",
                title: "La enfermedad ha sido desactivada :)",
              });
            } else {
              const Toast = Swal.mixin({
                toast: true,
              ...
              Toast.fire({
                type: "error",
                title: "Ups! No se pudo desactivar la enfermedad",
              });
            }
            tabla.ajax.reload();
          }
...
//Función para activar la enfermedad
function activar(idenfermedad) {
  ...
  swalWithBootstrapButtons
    .fire({
      title: "¿Estas seguro?",
      text: "¿Quieres activar esta enfermedad?",
      type: "warning",
      showCancelButton: true,
    ...
    .then((result) => {
      if (result.value) {
        $.post(
          "../controladores/enfermedad.php?op=activar",
          { idenfermedad: idenfermedad },
          function (e) {
            if (e == "true") {
              const Toast = Swal.mixin({
              ...
              Toast.fire({
                type: "success",
                title: "La enfermedad ha sido activada :)",
              });
            } else {
              const Toast = Swal.mixin({
                toast: true,
              ...
              Toast.fire({
                type: "error",
                title: "Ups! No se pudo activar la enfermedad",
              });
            }
            tabla.ajax.reload();
          }
...
function limpiar() {
  $("#enfermedad").val("");
  $("#enfermedad").removeClass("is-invalid");
  $("#estatus").val("");
