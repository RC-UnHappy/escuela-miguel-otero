...
(function () {
  ...
  $.post(
    ...
    function (response) {
      $("#listaDiversidad").html(response);
    }
  );

  $.post(
    "../controladores/enfermedad.php?op=traer-enfermedades",
    function (response) {
      $("#listaEnfermedad").html(response);
    }
  );

  $('input[name="todas_vacunas"]').on("click", function (e) {
    if (e.target.value == "1") {
      $("#vacunas").show();
    } else {
      $("#vacunas").hide();
    ...
    if (data.length != 0) {
      data.forEach(function (indice) {
        planificacion +=
          ...
          indice.seccion +
          '" - ' +
          indice.nombre_docente +
          " " +
          indice.apellido_docente +
          "</option>";
      });
    } else {
      planificacion =
        '<option value="">No hay planificaciones activas</option>';
    }
  ...
  $("#planificaciones_general").on("change", function () {
    ...
    if (idplanificacion != "") {
      ...
      $("#botonReporte").html(
        '<a target="_blank" href="../reporte/aspecto-fisiologico.php?idplanificacion=' +
          idplanificacion +
          '"><button class="btn btn-outline-primary btn-pill shadow-sm" title="Generar reporte de los datos fisiológicos"><i class="fa fa-file"></i> Reporte</button></a>'
      );
    } else {
      listar();
      $("#botonReporte").html("");
...
function traerPlanificaciones() {
  let planificaciones = $.post(
    "../controladores/aspecto-fisiologico.php?op=traerplanificaciones"
  );
...
function traerEstudiantes(idplanificacion = null) {
  let estudiantes = $.post(
    "../controladores/aspecto-fisiologico.php?op=traerestudiantes",
...
function traerLiterales() {
  let literales = $.post(
    "../controladores/boletin-final.php?op=traerliterales"
  );
  return literales;
}
...
function mostrar(idaspectofisiologico) {
  $.post(
    "../controladores/aspecto-fisiologico.php?op=mostrar",
    { idaspectofisiologico: idaspectofisiologico },
    function (data) {
      data = JSON.parse(data);
      ...
      $("#estudiante").html(
        '<option value="' +
          data.idestudiante +
          '">' +
          capitalize(data.p_nombre_estudiante) +
          " " +
          capitalize(data.s_nombre_estudiante) +
          " " +
          capitalize(data.p_apellido_estudiante) +
          " " +
          capitalize(data.s_apellido_estudiante) +
          "</option>"
      );
      $("#estudiante").selectpicker("refresh");
      ...
      if (data.c == 1) $("#cSi").attr("checked", true);
      else $("#cNo").attr("checked", true);

      if (data.alimentos == 1) $("#alimentosSi").attr("checked", true);
      else $("#alimentosNo").attr("checked", true);

      if (data.utiles == 1) $("#utilesSi").attr("checked", true);
      else $("#utilesNo").attr("checked", true);
...
function limpiar(idplanificacion = "") {
  ...
  planificaciones.then((data) => {
    ...
    if (data.length != 0) {
      ...
      planificacion =
        '<option value="">Debe finalizar todos los lapsos académicos</option>';
    }
    $("#planificacion").html('<option value="">Seleccione</option>');
    $("#planificacion").append(planificacion);
    $("#planificacion").selectpicker("refresh");
