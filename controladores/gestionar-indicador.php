    ...
    if (!empty($idplanificaciones)){
      ...
      if ($rspta->num_rows != 0) {
        while ($reg = $rspta->fetch_object()) {
          ...
          ( ( isset($_SESSION['permisos']['gestionar-indicador']) && 
              in_array('editar' , $_SESSION['permisos']['gestionar-indicador']) ) ?

            '<button class="btn btn-outline-primary " title="Editar" onclick="mostrar('.$reg->id.')" data-toggle="modal" data-target="#gestionarIndicadoresModal"><i class="fas fa-edit"></i></button>' : '' ).

          // Se verifica que tenga el permiso de eliminar para mostrar o no el botón
          ( ( isset($_SESSION['permisos']['gestionar-indicador']) && 
                      in_array('activar-desactivar' , $_SESSION['permisos']['gestionar-indicador']) ) ?
