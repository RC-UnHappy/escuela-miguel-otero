...
switch ($_GET['op']) {
    ...
    $idperiodo_actual = !empty($idperiodo_actual) ? $idperiodo_actual['id'] : '';
        ...
        if ($idperiodo_actual == $reg->idperiodo_escolar && $reg->estatus == 'Planificado') {
          ...
          ( ( isset($_SESSION['permisos']['pic']) && 
              in_array('activar-desactivar' , $_SESSION['permisos']['pic']) ) ?
          ...
          ( ( isset($_SESSION['permisos']['pic']) && 
              in_array('editar' , $_SESSION['permisos']['pic']) ) ?

          '<button class="btn btn-outline-primary" title="Editar" onclick="mostrar('.$reg->id.')" data-toggle="modal" data-target="#picModal"><i class="fa fa-edit"></i></button>' : '');
