    ...
    if (!empty($idplanificacion) && !empty($idestudiantes) && !empty($lapso)){
      ...
      if ($rspta->num_rows != 0) {
        ...
        while ($reg = $rspta->fetch_object()) {
          ...
          $data[] = array(
          '0' => ($solo_uno) == 0 ? '<a target="_blank" href="../reporte/boletin_informativo.php?idplanificacion='.$idplanificacion.'&lapso='.$lapso.'&idestudiante='.$idestudiantes.'"> <button class="btn btn-primary" title="Boletín Informativo"><i class="fa fa-file"></i></button></a>' : '',
          '1' => $reg->materia,
          '2' => $reg->indicador,
          '3' => $reg->nota);
