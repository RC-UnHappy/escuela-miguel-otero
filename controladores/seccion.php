			...
			while ($reg = $rspta->fetch_object()) {
				...
				$data[] = array('0' => 
					...
					// Se verifica que tenga el permiso de editar para mostrar o no el botón
					 ( ( isset($_SESSION['permisos']['seccion']) && 
                  	  in_array('editar' , $_SESSION['permisos']['seccion']) ) ?

					'<button class="btn btn-outline-primary " title="Editar" onclick="mostrar('.$reg->id.')" data-toggle="modal" data-target="#seccionModal"><i class="fas fa-edit"></i></button>' : '').

					// Se verifica que tenga el permiso de eliminar para mostrar o no el botón
					( ( isset($_SESSION['permisos']['seccion']) && 
                  	  in_array('activar-desactivar' , $_SESSION['permisos']['seccion']) ) ?
